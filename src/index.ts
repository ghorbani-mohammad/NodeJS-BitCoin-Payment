...
import type { BtcpayWebhookEvent, CreateInvoiceBody } from "./types";
...
app.post("/api/webhooks/btcpay", express.raw({ type: "application/json" }), async (req: Request, res: Response) => {
  try {
    ...
    const event: BtcpayWebhookEvent = JSON.parse(req.body.toString());
